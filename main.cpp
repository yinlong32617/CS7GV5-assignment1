...
bool FPView = false; // first person view
...
void display() {
	...
	if (FPView) {
		...
		view = look_at(vec3(main_x - 150, main_y, 10.0f), vec3(main_x, main_y, 0.0f), vec3(0, 0, 1));
	}
	else {
...
void updateScene() {
	...
	//cout << "Pitch = " << rotate_y << ", Roll = " << rotate_x << ", Yaw = " << rotate_z << endl;
...
void keypress(unsigned char key, int x, int y) {
	...
	if (key == 'x') {  // Switch between first-person and third-person views
		FPView = !FPView;
	}
	...
	// Observe gimbal-lock occurring when 2 axes are aligned
	if (key == 'g') { 
		rotate_y = 90;  //Gimbal Lock means we lost 1 degree of freedom, the plane cann't yaw
						//when you yaw the plane you actually roll the plane
	}
	if (key == 'h') {
		rotate_x = 0;
		rotate_y = 0;   // no Gimbal Lock
		rotate_z = 0;
	}
}
